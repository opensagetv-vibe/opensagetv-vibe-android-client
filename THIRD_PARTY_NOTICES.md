...
| `extension-media3-ffmpeg-1.11.0.aar` | AndroidX Media3 FFmpeg audio decoder extension and an FFmpeg 6.0 `libavcodec`/`libavutil`/`libswresample` subset | The Java/JNI extension follows AndroidX Media's Apache-2.0 terms. The pinned component-only build enables only the documented audio decoders and disables programs and unrelated libraries without enabling GPL or nonfree code. Its JNI library is renamed to `libmedia3ffmpegJNI.so` so it can coexist with the legacy Exo extension; Vibe explicitly disables the module's experimental video renderer. The FFmpeg portion retains LGPL 2.1-or-later obligations. |
