...
class FullscreenControllerTests(unittest.TestCase):
    ...
    def test_stock_server_preview_promotion_is_conditional_and_generation_safe(self):
        ...
        debug_state = (ROOT / "source/dev/android-tv/src/debug/java/"
                       "opensagetv/vibe/miniclient/android/tv/debug/DebugStateProvider.java").read_text(encoding="utf-8")
        ...
        self.assertIn("FULLSCREEN_PROMOTION_DELAY_MS = 350", base)
        self.assertIn("FULLSCREEN_PROMOTION_MAX_CHECKS = 24", base)
        self.assertIn("fullscreenPromotionCheckScheduled", base)
        self.assertIn("if (!playerReady || player == null)", base)
        self.assertIn("scheduleFullscreenPromotionCheck();", base)
        self.assertIn("isPlaybackMenu(menuName) && !embeddedPreview", base)
        self.assertIn("if (FullscreenPlaybackPolicy.isPlaybackMenu(menuName))", base)
        self.assertIn("fullscreenPromotionCommandCount", base)
        self.assertIn("fullscreenPromotionLastDecision", base)
        self.assertIn("FULLSCREEN_PROMOTION_STABLE_CHECKS = 6", base)
        self.assertIn("waiting_for_stable_preview", base)
        self.assertIn("waiting_for_stable_fullscreen", base)
        self.assertIn("fullscreenPromotionCommandCount=", debug_state)
        self.assertIn("fullscreenPromotionLastDecision=", debug_state)
        self.assertIn("fullscreenPromotionStablePreviewCount=", debug_state)
        self.assertIn("fullscreenPromotionStableFullscreenCount=", debug_state)
        self.assertIn("videoInfo.destRect.width", base)
        self.assertIn("videoInfo.destRect.height", base)
        self.assertGreaterEqual(base.count("playerReady = false;"), 2)
