# Player Test Harness v0.5.47

## Per-step watchdog correction

The default `--watchdog-ms 180000` is applied independently to every media action. Each action starts a fresh timer; the watchdog is not a total process budget. Android debug snapshots advertise `maxRecoveryWatchdogMs=300000`, and MCP verifies that the installed APK can honor the requested watchdog. The ADB transport timeout is also extended beyond each requested recovery window.

Absolute seek inherits `--watchdog-ms` by default. Use `--seek-timeout-s N` only when intentionally overriding the absolute-seek step.

Search recording startup now uses `ff -> right -> play_pause -> down -> play_pause` after the recording text is entered.
