...
final class PlaybackHealthProbe
{
    ...
    private static Snapshot captureOnMainThread(MiniPlayerPlugin topLevelPlayer)
    {
        ...
        if (dataSource != null)
        {
            ...
            out.dataSourceOpenCount = invokeLongOptional(dataSource, "getOpenCount", -1);
            out.dataSourceOpenWaitMs = invokeLongOptional(dataSource, "getOpenWaitMs", -1);
            out.dataSourceLastOpenPosition = invokeLongOptional(dataSource, "getLastOpenPosition", -1);
            out.dataSourceNetworkReadCount = invokeLongOptional(dataSource, "getNetworkReadCount", -1);
            out.dataSourceNetworkReadRequestedBytes = invokeLongOptional(dataSource, "getNetworkReadRequestedBytes", -1);
            out.dataSourceNetworkReadBytes = invokeLongOptional(dataSource, "getNetworkReadBytes", -1);
            out.dataSourceNetworkReadWaitMs = invokeLongOptional(dataSource, "getNetworkReadWaitMs", -1);
            out.dataSourceNetworkReadMaxRequestedBytes = invokeLongOptional(dataSource, "getNetworkReadMaxRequestedBytes", -1);
            out.dataSourceNetworkReadErrors = invokeLongOptional(dataSource, "getNetworkReadErrors", -1);
            out.dataSourceNetworkLastReadPosition = invokeLongOptional(dataSource, "getNetworkLastReadPosition", -1);
        }
