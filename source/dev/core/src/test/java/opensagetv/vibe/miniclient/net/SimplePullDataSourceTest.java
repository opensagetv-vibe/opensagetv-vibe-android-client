...
public class SimplePullDataSourceTest {
    ...
    @Test
    public void testReadAtExactEndReturnsEofWithoutServerRead() throws Exception {
        SimplePullDataSource pds = new SimplePullDataSource();
        pds.remoteReader = new DataInputStream(new MockInputStream(100));
        ...
        assertEquals(-1, pds.read(100, new byte[16], 0, 16));
        assertEquals(0, commands.size());
    }

    @Test
    public void testFinalReadIsBoundedToKnownRemainingBytes() throws Exception {
        SimplePullDataSource pds = new SimplePullDataSource();
        pds.remoteReader = new DataInputStream(new MockInputStream(100));
        ByteArrayOutputStream commands = new ByteArrayOutputStream();
        pds.remoteWriter = commands;
        ...
        assertEquals(5, pds.read(95, new byte[16], 0, 16));
        assertEquals("READ 95 5\r\n", commands.toString("ISO-8859-1"));
    }

    @Test
    public void testWaitForGrowthRefreshesMediaServerSize() throws Exception {
        SimplePullDataSource pds = new SimplePullDataSource();
        pds.remoteReader = new DataInputStream(new ByteArrayInputStream("150\r\n".getBytes("ISO-8859-1")));
        ByteArrayOutputStream commands = new ByteArrayOutputStream();
        pds.remoteWriter = commands;
        pds.opened = true;
        pds.size = 100;
