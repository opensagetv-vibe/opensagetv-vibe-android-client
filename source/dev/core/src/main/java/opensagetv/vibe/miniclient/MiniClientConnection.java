...
public class MiniClientConnection implements SageTVInputCallback
{
    ...
    */
    ...
    private ConnectionEventRouter eventRouterThread;
    ...
    private volatile long subtitleCallbackEventCount;
    private volatile long subtitleCallbackByteCount;
    ...
    private final ConnectionReconnectState reconnectState = new ConnectionReconnectState();
    private final ConnectionLifecycleDiagnostics.Trace connectionDiagnostics;
    private final ConnectionWorkerOwner connectionWorkers = new ConnectionWorkerOwner();
    private final ConnectionProtocolStreams protocolStreams = new ConnectionProtocolStreams();
    private final ConnectionFileTransferOwner fileTransfers;

    private MenuHint menuHint = new MenuHint();
    private ConnectionCapabilityProfile capabilityProfile;

    public MiniClientConnection(MiniClient client, String myID, ServerInfo msi, ILogger log)
    {
        this.log = log;
        this.client = client;
        fileTransfers = new ConnectionFileTransferOwner(log);
        ...
        this.msi = msi;
        connectionDiagnostics = ConnectionLifecycleDiagnostics.begin(msi.address);
        usesAdvancedImageCaching = false;
    }

    /** Returns the effective client ID used by this live SageTV connection. */
    ...
    private java.net.Socket EstablishServerConnection(int connType) throws java.io.IOException {
        ...
        try {
            connectionDiagnostics.lifecycle("socket_" + connType + "_connect_started");
            ...
            connectionDiagnostics.lifecycle("socket_" + connType + "_accepted");
            sake.setSoTimeout(0);
            return sake;
        } catch (java.io.IOException e) {
            connectionDiagnostics.lifecycle("socket_" + connType + "_failed");
    ...
    public void connect() throws java.io.IOException {
        ...
        connectionDiagnostics.lifecycle("codec_discovery_complete");
        ...
        while (connectionWorkers.mediaSocket() == null) {
            connectionWorkers.setMediaSocket(EstablishServerConnection(1));
            if (connectionWorkers.mediaSocket() == null) {
        ...
        connectionDiagnostics.lifecycle("media_socket_ready");

        log.logInfo("Connecting to ui server at: " + msi);
        while (connectionWorkers.gfxSocket() == null) {
            connectionWorkers.setGfxSocket(EstablishServerConnection(0));
            if (connectionWorkers.gfxSocket() == null) {
        ...
        connectionDiagnostics.lifecycle("gfx_socket_ready");

        alive = true;
        connectionDiagnostics.alive(true);
        client.setCurrentConnection(this);
        connectionDiagnostics.lifecycle("connection_published");
        Thread t = new Thread("Media-" + msi.address) {
            public void run() {
                connectionDiagnostics.workerStarted("media");
                connectionWorkers.markMediaWorkerStarted();
                try {
                    MediaThread();
                } finally {
                    connectionDiagnostics.workerStopped("media");
                }
            }
        };
        connectionWorkers.retainMedia(t);
        connectionDiagnostics.lifecycle("media_worker_launch");
        t.start();
        if (!connectionWorkers.awaitMediaWorkerStarted(2000L)) {
            close();
            throw new java.io.IOException("Timed out waiting for SageTV media worker readiness");
        }
        connectionDiagnostics.lifecycle("media_before_gfx_ready");
        t = new Thread("GFX-" + msi.address) {
            public void run() {
                connectionDiagnostics.workerStarted("gfx");
                try {
                    GFXThread();
                } finally {
                    connectionDiagnostics.workerStopped("gfx");
                }
            }
        };
        connectionWorkers.retainGfx(t);
        connectionDiagnostics.lifecycle("gfx_worker_launch");
        t.start();
    ...
    {
        capabilityProfile = ConnectionCapabilityProfile.discover(client, log);
    }
    ...
    public synchronized void close() {
        if (!connectionWorkers.beginClose())
            return;
        connectionDiagnostics.closeRequested();
        alive = false;
        connectionDiagnostics.alive(false);
        try {
        ...
        // Socket closure above unblocks Media/GFX readers. Only the queue-owned
        // event worker is interrupted directly; then wait on one shared,
        // bounded deadline and never join the calling worker.
        connectionWorkers.interruptEventRouter();
        connectionWorkers.awaitStopped(ConnectionWorkerOwner.DEFAULT_CLOSE_WAIT_MS);

        connectionDiagnostics.closeComplete();

    }

    private synchronized boolean installGfxReconnectSocket(java.net.Socket socket) {
        if (!alive || socket == null) {
            closeQuietly(socket);
            return false;
        }
        connectionWorkers.setGfxSocket(socket);
        return true;
    }

    private synchronized boolean installMediaReconnectSocket(java.net.Socket socket) {
        if (!alive || socket == null) {
            closeQuietly(socket);
            return false;
        }
        connectionWorkers.setMediaSocket(socket);
        return true;
    }
    ...
    private void GFXThread()
    {
        ...
        final GfxFrameExchange gfxFrames = new GfxFrameExchange(connectionDiagnostics);
        ...
        try
        {
            ...
            eventRouterThread = new ConnectionEventRouter("EVTRouter", connectionDiagnostics,
                    new ConnectionEventRouter.DispatchState()
                    {
                        @Override
                        public boolean shouldDispatch()
                        {
                            return reconnectState.shouldDispatchEvents();
                        }
                    }, log);
            connectionWorkers.retainEventRouter(eventRouterThread);
            connectionDiagnostics.lifecycle("event_router_launch");
            eventRouterThread.start();

            ConnectionProtocolStreams.OpenStreams openedStreams =
                    protocolStreams.open(connectionWorkers.gfxSocket());
            eventChannel = openedStreams.eventOutput;
            gfxIs = openedStreams.gfxInput;
            zipMode = false;
            ...
            Thread gfxReadThread = new Thread("GFXRead")
            {
                public void run()
                {
                    connectionDiagnostics.workerStarted("gfx_read");
                    ...
                    while (alive)
                    {
                        ...
                        try
                        {
                            if (zipMode && !enabledzip)
                            {
                                ...
                                com.jcraft.jzlib.ZInputStream zs = new com.jcraft.jzlib.ZInputStream(connectionWorkers.gfxSocket().getInputStream(),true);
                        ...
                        catch (Exception e)
                        {
                            if (reconnectState.canReconnect(alive, encryptEvents))
                            {
                                ...
                                connectionDiagnostics.reconnect("started");
                                enabledzip = false;
                                ...
                                try
                                {
                                    connectionWorkers.gfxSocket().close();
                                }
                                catch (Exception e1) { }
                                ...
                                try
                                {
                                    ...
                                    ConnectionProtocolStreams.OpenStreams reconnectedStreams =
                                            protocolStreams.open(connectionWorkers.gfxSocket());
                                    eventChannel = reconnectedStreams.eventOutput;
                                    myStream = gfxIs = reconnectedStreams.gfxInput;
                                ...
                                catch (Exception e1)
                                {
                                    ...
                                    connectionDiagnostics.reconnect("failed");
                                    ...
                                    if (client!=null)
                                    {
                                        client.eventbus().post(new ConnectionLost(reconnectState.isReconnecting()));
                                    }
                                    gfxFrames.publishFailure(e);
                                    connectionDiagnostics.workerStopped("gfx_read");
                                    return;
                                }
                                reconnectState.finish();
                            }
                        ...
                        gfxFrames.publish(gfxCmds, cmdbuffer, gfxCmds[0] & 0xFF);
                    }
                    connectionDiagnostics.workerStopped("gfx_read");
                }
            };
            connectionWorkers.retainGfxRead(gfxReadThread);
            ...
            while (alive)
            {
                ...
                if (frame.failure != null)
                    throw frame.failure;
                cmd = frame.header;
                cmdbuffer = frame.body;

                command = (cmd[0] & 0xFF);
                len = ((cmd[1] & 0xFF) << 16) | ((cmd[2] & 0xFF) << 8) | (cmd[3] & 0xFF);
                connectionDiagnostics.gfxDispatch(command);
                ...
                if (command == DRAWING_CMD_TYPE) // GFX cmd
                {
                    ...
                    if (command == GFXCMD_MEDIA_RECONNECT)
                    {
                        ...
                        try
                        {
                            connectionWorkers.mediaSocket().close();
                        }
                        catch (Exception e) { }
                    }
                    else
                    {
                        ...
                        {
                            if (reconnectState.markFirstFrameStarted())
                                connectionDiagnostics.lifecycle("first_frame_started");
                        }
                        ...
                        if (hasret[0] != 0)
                        {
                            ...
                            try
                            {
                                synchronized (eventChannel)
                                {
                                    ...
                                    eventChannel.flush();
                                    connectionDiagnostics.gfxReply(command);
                                }
                            }
                            catch (Throwable e)
                            {
                ...
                else if (command == GET_PROPERTY_CMD_TYPE) // get property
                {
                    ...
                    else if ("MEDIA_STATE_URL".equals(propName))
                    {
                        // Advertise support for the backward-compatible stv URL
                        // fragment carrying real completed/growing media state.
                        propVal = "TRUE";
                    }
                    else if ("DVD_REMOTE_NAV".equals(propName))
                    {
                        // Commands 32-37 are implemented independently of the
                        // selected media representation. Do not hide working
                        // native DVD navigation merely because an optional
                    ...
                    else if ("FRAME_STEP".equals(propName))
                    {
                        propVal = opensagetv.vibe.miniclient.video.PlaybackFrameStepPolicy.shouldAdvertise(
                                client.properties().getStreamingMode(),
                                client.properties().getString(PrefStore.Keys.default_player, "exoplayer"),
                    ...
                    {
                        propVal = opensagetv.vibe.miniclient.video.LegacySubtitleCallbackPolicy
                                .shouldAdvertise(client.properties().getString(
                                        PrefStore.Keys.default_player, "exoplayer"))
                                ? "TRUE" : "FALSE";
                    }
                    ...
                    else if ("AUDIO_CODECS".equals(propName))
                    {
                        ...
                        else
                        {
                            ...
                            propVal = capabilityProfile.audioCodecs();
                            if (extra_codecs != null)
                            {
                                propVal += "," + extra_codecs;
                            }
                    ...
                    else if ("PUSH_AV_CONTAINERS".equals(propName))
                    {
                        ...
                        else if (canDoPullStreaming && StreamingModePolicy.isForcedPull(client.properties().getStreamingMode()))
                        {
                        ...
                        {
                            if (!capabilityProfile.hasPushFormats())
                            {
                                propVal = "NONE";
                            }
                            else
                    ...
                    else if ("PULL_AV_CONTAINERS".equals(propName))
                    {
                        ...
                        */
                        ...
                        else
                        {
                            ...
                            if (StreamingModePolicy.isForcedPull(client.properties().getStreamingMode()))
                            {
                                propVal = capabilityProfile.pushFormats() + "," + capabilityProfile.pullFormats();
                            }
                            else
                            {
                                propVal = capabilityProfile.pullFormats();
                            }
                        }
                    }
                    ...
                    {
                        propVal = capabilityProfile.profileProperty("GFX_DRAWMODE", "FULLSCREEN");
                    }
                    ...
                    if ((propVal == null || propVal.isEmpty()) && capabilityProfile != null)
                    {
                        propVal = capabilityProfile.profileProperty(propName, propVal);
                    }
                ...
                else if (command == SET_PROPERTY_CMD_TYPE) // set property
                {
                    ...
                    synchronized (eventChannel)
                    {
                        ...
                        {
                            reconnectState.setAllowed("TRUE".equalsIgnoreCase(new String(cmdbuffer, 4 + nameLen, valLen)));
                            propVal = String.valueOf(reconnectState.isAllowed());
                            retval = 0;
                        }
                        ...
                        else if ("VIDEO_CC_STATE".equals(propName))
                        {
                            propVal = new String(cmdbuffer, 4 + nameLen, valLen);
                            try
                            {
                                int ccState = Integer.parseInt(propVal);
                                if (ccState < 0)
                                    throw new NumberFormatException("negative caption state");
                                if (myMedia == null)
                                    throw new IllegalStateException("media command handler is not initialized");
                                myMedia.setSageTvClosedCaptionState(ccState);
                                retval = 0;
                                log.logDebug("SageTV STV closed-caption state is now: " + ccState);
                            }
                            catch (RuntimeException e)
                            {
                                retval = 1;
                                log.logError("Invalid VIDEO_CC_STATE property: " + propVal, e);
                            }
                        }
                        else if ("VIBE_CURRENT_CHANNEL".equals(propName))
                        {
                            propVal = new String(cmdbuffer, 4 + nameLen, valLen);
                            if (propVal.matches("[0-9]+(?:\\.[0-9]+)?"))
                            {
                                vibeCurrentChannel = propVal;
                                vibeChannelAckSequence++;
                                retval = 0;
                            }
                            else
                            {
                                retval = 1;
                                log.logError("Invalid VIBE_CURRENT_CHANNEL property: " + propVal);
                            }
                        }
...
//        }
    ...
    public void postRepaintEvent(final int x, final int y, final int w, final int h) {
        if (reconnectState.isReconnecting())
            return;

        if (eventRouterThread == null || eventRouterThread.queue == null || eventChannel == null) {
            return;
        }

        // Activity resume can request a repaint from Android's main thread. Keep every event-channel
        // socket write on the ordered event router so Android never rejects it as main-thread network I/O.
        eventRouterThread.enqueue(new Runnable() {
            @Override
            public void run() {
                synchronized (eventChannel) {
    ...
    /**
     * Monotonic count of positive channel acknowledgements received on this
     * MiniClient connection. Unlike MediaCmd flush counters, this survives a
     * player replacement during a live channel change.
     */
    ...
    public boolean isSubtitleCallbackEnabled() {
        return subSupport;
    }

    /** Successfully flushed legacy subtitle callback events on the event channel. */
    public long getSubtitleCallbackEventCount() {
        return subtitleCallbackEventCount;
    }

    /** Raw caption payload bytes successfully flushed on the event channel. */
    public long getSubtitleCallbackByteCount() {
        return subtitleCallbackByteCount;
    }
...
//					}
    ...
    private int startAsyncFSOperation(boolean download, int secureID, long fileOffset, long fileSize, java.io.File theFile) {
        ...
        try {
            ...
            if (!fileTransfers.start(download, sake, fsOut, fsIn, fileSize, raf))
                throw new java.io.IOException("Connection closed before remote FS transfer start");
        } catch (java.io.IOException e) {
            if (raf != null)
                try {
                    raf.close();
    ...
    private void MediaThread() {
        ...
        while (alive) {
            ...
            try {
                os = connectionWorkers.mediaSocket().getOutputStream();
                is = new java.io.DataInputStream(connectionWorkers.mediaSocket().getInputStream());
                while (alive) {
                    byte[] cmd = new byte[4];
                    int command, len;
                    int retval;
                    ...
                    command = (cmd[0] & 0xFF);
                    len = ((cmd[1] & 0xFF) << 16) | ((cmd[2] & 0xFF) << 8) | (cmd[3] & 0xFF);
                    connectionDiagnostics.mediaCommand(command);
                    if (cmdbuffer.length < len) {
                        cmdbuffer = new byte[len];
                    }
                ...
                is = null;
                connectionWorkers.closeAndClearMediaSocket();
            }
            if (!alive)
                break;
            try {
                connectionDiagnostics.reconnect("started");
                java.net.Socket reconnectSocket = EstablishServerConnection(1);
                if (!installMediaReconnectSocket(reconnectSocket))
                    break;
                connectionDiagnostics.reconnect("succeeded");
            } catch (Exception e) {
                connectionDiagnostics.reconnect("failed");
