...
public class ImageCache
{
    ...
    {
        long requestedBytes = imageBytes(width, height);
        boolean canDo = requestedBytes >= 0
                && requestedBytes <= imageCacheLimit - imageCacheSize;
        if (!canDo)
        {
            log.logDebug("Can't cache {" + width + " }x{" + height + "} ("
                    + (requestedBytes < 0 ? "invalid" : Utils.toMB(requestedBytes))
                    + "mb).  Not enough room.  Cache Size: " + Utils.toMB(imageCacheSize)
                    + "; Cache Limit: " + Utils.toMB(imageCacheLimit));
        }
        return canDo;
    }
    ...
    public void put(int imghandle, ImageHolder<?> img, int width, int height) {
        ...
        imageCacheSize += bytes;

        if (VerboseLogging.DETAILED_IMAGE_CACHE)
        {
            log.logDebug("Added " + imghandle + " with size: " + width + "x" + height + " (" + Utils.toMB(bytes) + "mb); Cache " + Utils.toMB(imageCacheSize) + "mb/" + Utils.toMB(imageCacheLimit)+ "mb)");
        }
    }

    public boolean makeRoom(int width, int height)
    {
        long requestedBytes = imageBytes(width, height);
        if (requestedBytes < 0 || requestedBytes > imageCacheLimit)
        {
            log.logError("ERROR invalid or oversized image allocation " + width + "x" + height);
            return false;
        }
        if (VerboseLogging.DETAILED_IMAGE_CACHE)
        {
        ...
        while (requestedBytes > imageCacheLimit - imageCacheSize)
        {
    ...
    public void unloadImage(int handle) {
        ...
        if (bi != null) {
            long bytes = imageBytes(bi.getWidth(), bi.getHeight());
            if (bytes > 0)
                imageCacheSize = Math.max(0, imageCacheSize - bytes);
            if (VerboseLogging.DETAILED_IMAGE_CACHE) {
    ...
    /**
     * Evict one ordinary LRU image for a single allocation-recovery attempt.
     * Surfaces and other renderer-owned objects are not in {@code lruImageMap}
     * and are therefore never selected here.
     */
    public boolean evictOldestDisposableImage()
    {
        int oldestImage = getOldestImage();
        if (oldestImage == 0)
            return false;
