...
public class BufferedPullDataSource extends SimplePullDataSource {
    ...
    /**
     * Creates a buffered SageTV pull source with a caller-selected network read buffer.
     * The default remains 32 KiB for compatibility with IJK/System paths; Exo-based
     * pull playback can use a larger buffer to reduce synchronous READ round trips
     * while scanning MPEG-TS timestamps after a seek.
     */
    public BufferedPullDataSource(String host, int bufferSize) {
        super(host);
        this.bufferSize = Math.max(MAX_BUFFER, bufferSize);
    }
    ...
    public int read(long position, byte[] buffer, int offset, int len) throws IOException {
        ...
        int sanity = 0;
        while (true) {
            read = bufferedRead(position + total, buffer, offset + total, Math.min(len - total, bufferSize));
    ...
    int bufferedRead(long position, byte[] buffer, int offset, int len) throws IOException {
        ...
        if (bytesRead == 0 || Math.abs(position - lastBufferStartPos) > bufferSize || bufPos + len > bufferSize || bufPos + len > bytesRead) {
    ...
    int fillBuffer(long position) throws IOException {
        return super.read(position, _buffer, 0, bufferSize);
    }
