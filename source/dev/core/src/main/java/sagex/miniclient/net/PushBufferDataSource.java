...
public class PushBufferDataSource implements ISageTVDataSource, HasPushBuffer
{
    ...
    // Exo/Media3 DataSource.read() must block until at least one byte is available.
    // Keep the historical non-blocking read() for IJK and other callers, and expose
    // an opt-in blocking path for Exo-based PUSH playback.
    ...
    public int readBlocking(long readOffset, byte[] bytes, int offset, int len) throws IOException
    {
        ...
        while (true)
        {
            ...
            int read = read(readOffset, bytes, offset, len);
            if (read != 0)
            {
                return read;
            }
            ...
            synchronized (dataAvailableMonitor)
            {
                try
                {
                    // Timed wait also protects against a notification racing between
                    // the empty read above and entry into this synchronized block.
                    dataAvailableMonitor.wait(10);
                }
