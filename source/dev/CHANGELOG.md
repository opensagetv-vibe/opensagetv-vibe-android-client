...
- Restored original generated/persisted client ID behavior and original per-server override handling.
- Removed hard-coded DEV001 enforcement; existing persisted IDs are preserved during upgrade.
- Re-enabled editable client ID settings and added debug-only CLI/MCP override support.
- `debugStatusVersion` 14.

# v0.5.69

- Media3 Pull uses CLOSEST_SYNC again and disables the harmful blind 10 s reprepare fallback.
- Legacy Exo2 keeps directional Pull sync seeking and now uses a 512 KiB SageTV Pull network block.
- Explicit GSY/System selection falls back to Media3 instead of immediately ending SageTV playback.
- debugStatusVersion 12.

# v0.5.67

- Pull seek policy: NEXT_SYNC forward / PREVIOUS_SYNC backward for >=2 s deltas; CLOSEST_SYNC for small seeks.
- Media3 Pull TS search restored to 8x baseline.
- One-shot 10 s Pull BUFFERING recovery reprepare for Media3 and Exo2.
- Media3/Exo2 Pull DataSource zero-byte contract hardening.
- Exo2 Pull network/open telemetry added.
- debugStatusVersion 11.

## Dev handoff v0.5.63 diagnostics

- Debug APK status v9 exposes SageTV seek target plus seek/flush/anchor timing.
- Added debug-only exact-event playback trap bridge/ring for MCP counter snapshots at player events.
- Release/desktop builds resolve the optional trap bridge to a no-op when the debug implementation is absent.

- Dev fresh-install Streaming Mode default changed from `fixed` to `dynamic`.
**Dev / Unreleased**
- Fixed Dev SageTV client identity to `DEV001` (`44:45:56:30:30:31`) so clean APK reinstalls reuse the same server-side client profile.
- Removed experimental Android player telemetry hooks while restoring the pre-telemetry playback baseline.
