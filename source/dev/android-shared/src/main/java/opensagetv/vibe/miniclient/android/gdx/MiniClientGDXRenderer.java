            ...
            try {
                ...
                if (batchBegun) {
                    try {
                        ...
                        // Activity/session teardown can dispose or invalidate the batch
                        // between queued instructions. Never let cleanup kill GLThread.
                        log.error("Unable to finish renderer batch during lifecycle transition", t);
                    }
                }
                renderQueue.clear();
            }
