...
import opensagetv.vibe.miniclient.android.opengl.shapes.Line;
import opensagetv.vibe.miniclient.android.opengl.shapes.LineRectangle;
...
//        2018-12-28 08:50:16.115 23328-23382 D: [GLThread 458] DEBUG - RECT: 965,66 292,157 - TL: 0 0 0.ff, BL:666666.ff, TR:  0 0 0.ff, BR: 666666.ff
    ...
    public void GFXCMD_INIT() {
        ...
        if (!readiness.awaitReady()) {
            log.debug("GFXCMD_INIT() ended before renderer readiness");
            return;
        }
...
//        }
    ...
    public MiniPlayerPlugin newPlayerPlugin(MiniClientConnection connection, String urlString) {
        ...
        PlayerBackend backend = PlayerBackend.fromPreference(
                ActivePlayerSessionOverrides.resolveBackend(client.properties().getString(
                        PrefStore.Keys.default_player, PlayerBackend.DEFAULT_PREFERENCE)));

        boolean switchPlayerOneTime = activity.isSwitchingPlayerOneTime();
        if (switchPlayerOneTime) {
            backend = backend.next();
        }

        PlayerBackend requestedBackend = backend;
        backend = PlayerFactory.resolveForUrl(requestedBackend, urlString);
        if (!switchPlayerOneTime
                && backend == PlayerBackend.MEDIA3
                && player instanceof Media3MediaPlayerImpl
                && ((Media3MediaPlayerImpl) player).canRetainForNextUrl(urlString))
        {
            log.info("Retaining Media3 player for compatible media replacement");
            return player;
        }
        if (player != null) player.free();
        DiscPlaybackPolicy.Resolution discPolicy = DiscPlaybackPolicy.resolve(
        ...
        if (mimRuntimeFallback)
        {
            ...
            AppUtil.message(activity.getContext().getString(
                    R.string.msg_disc_policy_fallback, discPolicy.reason));
                ...
                && discPolicy.effective == DiscPlaybackPolicy.Effective.UNAVAILABLE)
        {
            ...
            AppUtil.message(activity.getContext().getString(
                    R.string.msg_disc_old_server_native, discPolicy.reason));
        }
        if (backend != requestedBackend) {
            log.warn("DVD backend {} is not commissioned; using {}", requestedBackend.preferenceValue(), backend.preferenceValue());
            AppUtil.message(activity.getContext().getString(
                    R.string.msg_disc_backend_fallback,
                    requestedBackend.displayName(), backend.displayName()));
        }

        log.info("Using player backend: {} ({})", backend.preferenceValue(), backend.displayName());
        player = PlayerFactory.create(activity, backend);
        return player;
    }

    @Override
    public void setVideoBounds(opensagetv.vibe.miniclient.uibridge.Rectangle o, opensagetv.vibe.miniclient.uibridge.Rectangle o1) {
        if (logDetails)
