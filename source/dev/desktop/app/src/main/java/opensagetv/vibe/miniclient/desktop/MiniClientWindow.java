    ...
    public void showCanvas(Dimension screenSize) {
        if (canvas==null) {
            ...
            layers.getScene().setOnMouseMoved(event -> {
                ...
                client.getCurrentConnection().postMouseEvent(toSageMouseEvent(event, opensagetv.vibe.miniclient.uibridge.MouseEvent.MOUSE_MOVED));
            });

            layers.getScene().setOnMouseClicked(event -> {
                if (client.getCurrentConnection()==null) return;
                log.debug("Mouse Clicked", event);
                client.getCurrentConnection().postMouseEvent(toSageMouseEvent(event, opensagetv.vibe.miniclient.uibridge.MouseEvent.MOUSE_PRESSED));
                client.getCurrentConnection().postMouseEvent(toSageMouseEvent(event, opensagetv.vibe.miniclient.uibridge.MouseEvent.MOUSE_RELEASED));
                client.getCurrentConnection().postMouseEvent(toSageMouseEvent(event, opensagetv.vibe.miniclient.uibridge.MouseEvent.MOUSE_CLICKED));
            });
        }
    }

    private opensagetv.vibe.miniclient.uibridge.MouseEvent toSageMouseEvent(MouseEvent event, int evtType) {
        if (evtType == opensagetv.vibe.miniclient.uibridge.MouseEvent.MOUSE_CLICKED
                || evtType == opensagetv.vibe.miniclient.uibridge.MouseEvent.MOUSE_PRESSED
                || evtType == opensagetv.vibe.miniclient.uibridge.MouseEvent.MOUSE_RELEASED) {
            return new opensagetv.vibe.miniclient.uibridge.MouseEvent(event.getSource(), evtType, System.currentTimeMillis(), 16, (int) event.getSceneX(), (int) event.getSceneY(), 1, 1, 0);
        } else {
            return new opensagetv.vibe.miniclient.uibridge.MouseEvent(event.getSource(), evtType, System.currentTimeMillis(), 0, (int) event.getSceneX(), (int) event.getSceneY(), event.getClickCount(), event.getButton().ordinal(), 0);
        }
    }
