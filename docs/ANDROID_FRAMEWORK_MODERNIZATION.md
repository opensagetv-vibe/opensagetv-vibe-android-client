...
| `source/dev/core/src/main/java/opensagetv/vibe/miniclient/ServerDiscovery.java` | One named single-thread executor serializes asynchronous discovery requests and is recreated after shutdown; each attempt sends the stock packet to the IPv4 limited broadcast plus unique directed broadcasts from active interfaces | A replacement request or `close()` cancels pending work and closes the active socket; unusable interfaces fail independently; duplicate server replies are collapsed; `MiniClient.shutdown()` terminates the executor without making later discovery reject work | Host target-resolution/restart tests pass; retain the stock limited-broadcast fallback and physically verify directed discovery after network changes |
...
| `source/dev/android-shared/src/main/java/opensagetv/vibe/miniclient/android/ActivePlayerAdjustmentsDialog.java` | The Activity decor view posts one 75 ms UI-only callback to reopen the parent caption dialog after a nested selection closes | The callback retains no player work, performs no media I/O, and resolves the current Activity/player state again when it runs; Activity overlay teardown dismisses the resulting dialog | Keep this bounded to dialog navigation. It must never become a caption clock, polling loop, or playback scheduler |
| `source/dev/android-shared/src/main/java/opensagetv/vibe/miniclient/android/video/media3/Media3AvSyncTestDialog.java` | One main-looper Handler debounces encoded-test timestamp re-anchors after explicit Left/Right calibration input | Dismiss removes the pending callback, releases the standalone test player, restores the active program's prior mute state, and rejects duplicate teardown through an atomic guard | User-requested local calibration only; it must never run continuously or replace the active SageTV playback clock |
| `source/dev/android-shared/src/main/java/opensagetv/vibe/miniclient/android/CurrentVideoDiagnosticTest.java` | One main-looper Handler sequences the explicitly requested bounded cadence, pause/resume, and safe reversible-seek observations | Every delayed step rechecks the captured player/session; completion or cancellation restores the original position and play/pause state and rejects replacement sessions | Release-safe user diagnostic only; it never runs continuously, stores at most four redacted reports, and physical ONN acceptance passed all applicable checks |
...
| `source/dev/android-shared/src/main/java/opensagetv/vibe/miniclient/android/video/DisplayRefreshController.java` | One main-looper Handler applies display-mode selection and, only after a real DVD mode change, schedules a bounded local video-output refresh | Session replacement and teardown cancel pending settle work; the callback rechecks the DVD session before Media3 rebinds its Surface without replacing the player, datasource, or server stream | Host policy tests and stock-server physical acceptance pass; never use this timer to seek or reconstruct the transport |
...
| `source/dev/android-shared/src/main/java/opensagetv/vibe/miniclient/android/diagnostics/DiagnosticExportController.java` | One process-owned single-thread executor serializes user-requested bundle creation and bounded SMB upload; one main-looper Handler owns result dialogs/toasts | Operations use bounded local data and SMB timeouts; the executor lives only for the application process and never samples or writes from player callbacks | Keep on-request work serialized and off the playback/UI threads; never add per-line network logging or unbounded artifacts |
| `source/dev/android-shared/src/main/java/opensagetv/vibe/miniclient/android/diagnostics/DiagnosticSessionSpool.java` | One application-owned main-looper Handler schedules the 60-second Always-mode refresh; one serial I/O executor writes the protected pending spool and atomically refreshes SMB | Disabling Always or shutdown removes periodic callbacks; Activity background, STOP, error, and exit enqueue one bounded checkpoint; an atomic guard coalesces concurrent requests and pending files survive process death | Physical forced-stop/relaunch retry passes. Preserve app-process ownership, bounded retention/timeouts, pre-upload redaction, and nonblocking playback behavior |
